/**
 * ARMOUR G2 — Specs 页签（本页专属，不复用 XrosSeries/Xros5Mini/SpecSectionJsData）
 *
 * 结构与共享版逐行对齐，保证视觉零回归；相对共享版的差异只有三处：
 *   1. 去掉 ReplacementPodSpec —— 本页规格数据只用到 textWithImage / color 两种
 *      （连同它里面遗留的 console.log 一并去掉）
 *   2. 修复重复 key —— 共享版拿「行文本」拼 key，而本页装箱清单三个套装版本
 *      都含「1 x TYPE-C Cable」等相同行，必然撞车。改用数组下标。
 *   3. 修复 key 当 prop —— React 不会把 key 放进 props，共享版读到的恒为
 *      undefined。父级设 key 才是正确位置，子组件不再接收。
 *
 * 类名沿用 `.ui_v3-spec-content` 等既有约定，样式仍由 ui-v4/common.scss 提供
 * （该表同时承载全页 .to-top 动画，本就必须注册，见方案文档 §1）。
 */
...
export function TextWithImageSpec({
  ...
  const renderTextWithImage = (text, title) => {
    ...
    const altTitle = title || primaryProductTitle || 'ARMOUR G2';
    let hasImage = false;

    // partIndex 参与 key：装箱清单里跨套装版本会出现完全相同的行，
    // 用文本拼 key 必然重复，用下标才唯一
    const nodes = parts.map((part, partIndex) => {
      const image = parseImagePart(part);
      if (image) {
        hasImage = true;
        const keyBase = `${altTitle}-${partIndex}-img`;
      ...
      const keyBase = `${altTitle}-${partIndex}-p`;
...
export function ColorSpec({
  ...
  //----------------------------------------
  return (
    <ul className={`f ${isNoMaxWidthFinish ? 'ui_v3-no_max_width_finish' : ''}`}>
      <li className="f_24 f-d-bold">{itemTitle}</li>
...
export function ArmourG2SpecSection({isActive = true, data}) {
  if (!data) return null;
